# DF node specification -- `N_LARGE` (BOTTLE ROCKET 4.7.0 (BR/1.1, 61K))

Document: `spec/DF_NODE_SPEC.md` · Release: DF-PA21.2-1.0.0
Authority: `adapter/dfabric/nodes.py` (`LargeNodeAdapter`), `adapter/dfabric/witness.py`,
...
The container VM `Large.zip` -- the divergent 4.7.0 lineage: BR/1.1 ISA, 41 opcodes, ABI 2, LCTLC/1.2 (columned-lctl/4.3) compiled by the Python tool `tools/lctl430.py`, 60,945-byte C production boundary.

Runtime contract (from the package, restated): `BR/1.1 ISA (marker 21), ABI 2, Device ABI 1.0, LCTLC/1.2 source, BRIM/1 images; freestanding C11 core behind a HAL`.
...
| `WorkerGroup.group_id` | `G_BR11` | ISA lineage; images do not cross groups (see s6) |
| `Worker.worker_id` | `W_LARGE` | one worker == one running instance of this VM |
| `Worker.failure_domain` | `FD_LARGE` | the VM is its own OS process |
| `Worker.capabilities` | `classical`, `row_sequence_witness`, `bottle_rocket`, dialect token | measured, not declared |
| `Worker.limits` | `cpu_slots=1`, `memory_bytes=40000000`, `qpu_slots=0`, `ebit_budget=0` | memory measured per live VM |
...
On this node the program is emitted in **LCTLC/1.2 (columned-lctl/4.3, Python compiler tools/lctl430.py)** and reads back
`R2`. A 3-row lowering as actually emitted:

```
LCTLC/1.2
@unit id=df.example version=4.3.0 language=columned-lctl/4.3 isa=BR/1.1 br_image_version=10 br_request_caps=CONTROL|ARITH
@defaults mode=WRAP width=WIDE
ID│LANE│OP│OUT│CTRL│IN│ARG│META
W00001│exec│MOVI│R0│C0:CONTROL│_│imm=1469598103934665603│_
W00002│exec│MOVI│R1│C0:CONTROL│_│imm=1099511628211│_
W00003│exec│MUL│R0│C0:ARITH│R0›R1│_│_
W00004│exec│MOVI│R3│C0:CONTROL│_│imm=8701260594087458703│_
W00005│exec│XOR│R0│C0:ARITH│R0›R3│_│_
W00006│exec│MUL│R0│C0:ARITH│R0›R1│_│_
W00007│exec│MOVI│R3│C0:CONTROL│_│imm=15039870754224043385│_
W00008│exec│XOR│R0│C0:ARITH│R0›R3│_│_
W00009│exec│MUL│R0│C0:ARITH│R0›R1│_│_
W00010│exec│MOVI│R3│C0:CONTROL│_│imm=5535257408846457961│_
W00011│exec│XOR│R0│C0:ARITH│R0›R3│_│_
W00012│exec│MOV│R2│C0:CONTROL│R0│_│_
W00013│exec│HALT│_│C0:CONTROL│_│_│_
@end
```
...
Restated from the package's evidence/qualification ledgers (`status OPERATIONAL, scope host-reference`) and AUDIT_ERRATA.md (F9-F12); binding this VM to the fabric closes none of them:
