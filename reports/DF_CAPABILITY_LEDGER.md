# DF_Large (N_LARGE) -- capability ledger
...
Distribution: BLOCKED 3, BLOCKED_CAPABILITY_ABSENT 1, BLOCKED_EXTERNAL_AUTHORITY 1, OPERATIONAL 16, VERIFIED 1. `operational_without_evidence`: []; `operational_without_passing_gate`: [].
...
| `DFN-07` | the VM's own build and acceptance gate | `OPERATIONAL` | `make  (build + operational) ; make brctl-dev ; make size` reproduces from the delivered bytes with stock flags; the VM's own sums verify afterwards (RELEASE_CONTENTS.sha256 (892 entries)) | `N1`=PASS |
