...
typedef struct{
 uint64_t*regs,*tmp,*tmp2,*stack;uint8_t*fragments;uint32_t caps[BR_CAPS];uint8_t memory[BR_MEMORY_BYTES],mailbox[BR_MAILBOX_BYTES];br_insn code[BR_MAX_CODE];br_diag diag[BR_DIAG_RECORDS];br_device devices[BR_DEVICES];
 uint32_t ip,cc_,sp,flags,trap,status,lc_,ib_,sb_,hb_,su_,hu_,pg_,iv_,last_txid,cw_,ds_,dh_,ft_,fl_,pa_,le_;uint16_t fn_,ml_;uint8_t am_,cn_,iz_,as_,ks_,dn_,ia_,ii_;uint8_t ih_[32],ik_[32];uint8_t sa_;const br_hal*hal;void*hc_;
}br_vm;

void br_sha256(const uint8_t*,size_t,uint8_t[32]);
int br_vm_create(br_vm*,const br_hal*,void*);int br_vm_initialize(br_vm*);int br_vm_configure(br_vm*,const br_vm_config*);int br_vm_init(br_vm*);void br_vm_reset(br_vm*,int);int br_vm_load_code(br_vm*,const br_insn*,size_t);int br_vm_verify(br_vm*);int br_vm_start(br_vm*);int br_vm_step(br_vm*);int br_vm_run(br_vm*,uint32_t);int br_vm_suspend(br_vm*);int br_vm_resume(br_vm*);int br_vm_stop(br_vm*);int br_vm_fault(br_vm*,unsigned);int br_vm_save(br_vm*);int br_vm_recover(br_vm*);void br_vm_destroy(br_vm*);void br_vm_free(br_vm*);int br_vm_set_ik_(br_vm*,const uint8_t*,size_t);int br_vm_trap_frame(const br_vm*,br_trap_frame*);
...
uint64_t*br_vm_reg(br_vm*,unsigned);const uint64_t*br_vm_reg_const(const br_vm*,unsigned);int br_vm_status_code(const br_vm*);const char*br_status_name(int);int br_core_selftest(void);
#endif
