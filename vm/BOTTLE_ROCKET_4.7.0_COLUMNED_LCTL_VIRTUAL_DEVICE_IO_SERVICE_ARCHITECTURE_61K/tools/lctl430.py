...
V='4.3.0'; LANG='columned-lctl/4.3'; MAGIC='LCTLC/1.2'; HDR='ID│LANE│OP│OUT│CTRL│IN│ARG│META'
OPS=['NOP','MOVI','MOV','JMP','JZ','JNZ','HALT','ADD','SUB','MUL','DIVU','MODU','AND','OR','XOR','NOT','SHL','SHR','CMP','LOAD','STORE','PUSH','POP','SVC','NEG','BITTST','ASHR','ROL','ROR','LOADX','STOREX','BEQ','CALL','RET','JMPR','ENTER','LEAVE','TRAP','CAPQ','CHECKPOINT','YIELD']
...
SVC={'NOP':0,'STATUS':1,'REVOKE':2,'GRANT':3,'CONFIG':4,'SAVE':5,'DIAG':6,'SHA256':7,'VERIFY':8,'TRUST':9,'ENTROPY':10,'TIMER':11,'CONSOLE_WRITE':12,'CONSOLE_READ':13,'DEVICE_CALL':14,'YIELD':15,'STORAGE_READ':16,'STORAGE_WRITE':17,'MONOTONIC':18,'MAILBOX_PUT':19,'MAILBOX_GET':20,'DEVICE_ENUM':21}
SVC_CAP={0:'SERVICE',1:'SERVICE',2:'CONTROL',3:'CONTROL',4:'STATE',5:'STATE',6:'DIAG',7:'SERVICE',8:'SERVICE',9:'UPDATE',10:'SERVICE',11:'SERVICE',12:'SERVICE',13:'SERVICE',14:'SERVICE',15:'SERVICE',16:'STATE',17:'STATE',18:'STATE',19:'SERVICE',20:'SERVICE',21:'SERVICE'}
...
def parse_imm(s):
    if re.fullmatch(r'(?:0|[1-9][0-9]*|0x[0-9A-Fa-f]+)',s): return parse_uint(s)
    m=re.fullmatch(r'(0|[1-9][0-9]*|0x[0-9A-Fa-f]+)(<<|>>|[+*\-&|^])(0|[1-9][0-9]*|0x[0-9A-Fa-f]+)',s)
    if not m: die('non-canonical constant expression '+s)
    a,b=parse_uint(m.group(1)),parse_uint(m.group(3));op=m.group(2)
    if op=='+': v=a+b
    elif op=='-':
        if b>a: die('constant expression underflow')
        v=a-b
    elif op=='*': v=a*b
    elif op=='<<': v=a<<b if b<64 else U64+1
    elif op=='>>': v=a>>b if b<64 else 0
    elif op=='&': v=a&b
    elif op=='|': v=a|b
    else: v=a^b
    if v>U64: die('constant expression exceeds uint64')
    return v
...
def parse_arg(s):
    ...
    for t in parts:
        ...
        k,v=t.split('=',1)
        if k not in {'imm','target','mode','width','region','svc','offset','signed'} or not v or k in out: die('bad/duplicate ARG key '+k)
        out[k]=v
    return out
...
def opcap(op,svc=None):
    if op=='SVC': return SVC_CAP[svc]
    if op=='CHECKPOINT': return 'STATE'
    if op=='YIELD': return 'SERVICE'
    if op in {'LOAD','STORE','LOADX','STOREX'}: return 'MEMORY'
    if op in {'PUSH','POP','CALL','RET','ENTER','LEAVE'}: return 'STACK'
    if op in {'ADD','SUB','MUL','DIVU','MODU','AND','OR','XOR','NOT','SHL','SHR','CMP','NEG','BITTST','ASHR','ROL','ROR'}: return 'ARITH'
    return 'CONTROL'
...
def parse(path, executable_required=False):
    ...
    if exec_rows:
        req={'version':V,'language':LANG,'isa':'BR/1.1'}
...
def semantic(p):
    ...
    for i,r in enumerate(er):
        ...
        if 'imm' in a: imm=parse_imm(a['imm'])
        if 'target' in a:
        ...
        if 'svc' in a:
            ...
            if service is None:
                ...
                if service>=len(SVC_CAP): die('service id out of range')
            imm=service
        if 'offset' in a:
            flags=parse_uint(a['offset'])
            if flags>65535: die('offset/flags exceeds uint16')
        # ISA/ABI 1.1 operand contract
        arity={
          ...
          'NOT':(True,1),'SHL':(True,1),'SHR':(True,1),'CMP':(False,2),'LOAD':(True,0),'STORE':(False,1),'PUSH':(False,1),'POP':(True,0),'SVC':(True,None),
          'NEG':(True,1),'BITTST':(False,1),'ASHR':(True,1),'ROL':(True,1),'ROR':(True,1),'LOADX':(True,1),'STOREX':(False,2),
          'BEQ':(False,2),'CALL':(False,0),'RET':(False,0),'JMPR':(False,1),'ENTER':(False,0),'LEAVE':(False,0),'TRAP':(False,0),
          'CAPQ':(True,1),'CHECKPOINT':(False,0),'YIELD':(False,0)}[op]
        need_dst,nin=arity
        ...
        allowed={'mode','width'}
        if op in {'MOVI','TRAP'}: allowed|={'imm'}
        if op in {'JMP','JZ','JNZ','BEQ','CALL'}: allowed|={'target'}
        if op in {'SHL','SHR','BITTST','ASHR','ROL','ROR'}: allowed|={'imm'}
        if op in {'LOAD','STORE'}: allowed|={'imm','region'}
        if op in {'LOADX','STOREX'}: allowed|={'offset','region'}
        if op=='SVC': allowed|={'svc','offset','region'}
        if op in {'ADD','SUB','MUL','CMP','NEG','ASHR'}: allowed|={'signed'}
        if any(k not in allowed for k in a): die(f'{op}: ARG key not allowed')
        if op=='MOVI' and 'imm' not in a: die('MOVI requires imm')
        if op=='TRAP' and ('imm' not in a or not 1<=imm<=27): die('TRAP requires canonical trap id 1..27')
        if op in {'JMP','JZ','JNZ','BEQ','CALL'} and 'target' not in a: die(op+' requires target')
        if op in {'SHL','SHR','ASHR','ROL','ROR'} and ('imm' not in a or imm>0xffffffff): die(op+' shift/rotate immediate exceeds uint32')
        if op=='BITTST' and ('imm' not in a or imm>=WORD_BITS): die('BITTST bit index out of range')
        if op in {'LOAD','STORE'}:
            if 'imm' not in a or imm>MEM-8: die(op+' memory address out of range')
            if region!='MEM': die(op+' direct access requires region=MEM')
        if op in {'LOADX','STOREX'}:
            if 'offset' not in a: die(op+' requires offset')
            imm=parse_uint(a['offset'])
            if imm>MEM-8: die(op+' indexed offset out of range')
            if region!='MEM': die(op+' indexed access requires region=MEM')
        if op=='SVC' and 'svc' not in a: die('SVC requires svc')
        if a.get('signed','0') not in {'0','1'}: die('signed must be 0 or 1')
        if a.get('signed')=='1': flags|=0x8000
        ...
        if op=='SVC' and service==14 and region!='DEVICE': die('DEVICE_CALL service requires region=DEVICE')
        ...
        if r['meta']!='_':
            for t in r['meta'].split(';'):
                ...
                meta[k]=v
        insns.append({'index':i,'id':r['id'],'op':op,'opcode':OPI[op],'rd':rd,'ra':ra,'rb':rb,'cap_index':ci,'capability':capname,'mode':mode,'mode_id':MI[mode],'width':width,'imm':imm,'target':target,'region':region if op in {'LOAD','STORE','LOADX','STOREX','SVC'} else None,'service':service,'flags':flags,'line':r['line'],'meta':meta})
...
def verify_cfg(insns,u):
    ...
    for i,x in enumerate(insns):
        op=x['op']
        if op in {'HALT','RET','JMPR','YIELD'} or (op=='SVC' and x['service']==15): pass
        elif op=='JMP': edges[i]=[x['target']]
        elif op in {'JZ','JNZ','BEQ'}:
            edges[i]=[x['target']]
        ...
        else: die('program falls off end without HALT/RET/JMPR/YIELD')
    seen=set(); stack=[0]
    while stack:
        i=stack.pop()
    ...
    depth={0:0}; q=[0]
    while q:
        i=q.pop(0); dep=depth[i]; op=insns[i]['op']; nd=dep+(1 if op in {'PUSH','ENTER'} else -1 if op in {'POP','LEAVE'} else 0)
        if nd<0 or nd>STACK: die('stack effect violates 0..256 bound')
        for z in edges[i]:
            if z in depth and depth[z]!=nd: die('unbalanced explicit stack across CFG/loop')
    ...
    dfs(0);term=u.get('termination','halt')
    if cyc and term!='budgeted':die('loop requires termination=budgeted')
    if term=='yield' and not any(x['op']=='YIELD' or (x['op']=='SVC' and x['service']==15) for x in insns):die('termination=yield requires YIELD')

def brir(path):
    p=parse(path,True); ins,req=semantic(p)
    o={'format':'BRIR/1','compiler':'br-lctlc/'+V,'verifier':'br-lctlv/'+V,'language':LANG,'isa':'BR/1.1','unit':p['unit']['id'],'image_version':parse_uint(p['unit']['br_image_version']),'source_sha256':p['source_sha256'],'requested_caps':req,'required_caps':sum({CAP[x['capability']] for x in ins}), 'instructions':ins}
    return optimize(o)
...
def encode(o,factory=False):
    ...
    h=bytearray(HEADER); h[0:4]=b'BRIM'; h[4:7]=bytes([4,3,0]); h[7]=1 if factory else 0; h[8]=2;h[9]=21;h[10]=16;h[11]=16;h[12]=len(OPS);h[13]=4
    ...
    h[32:64]=hashlib.sha256(payload).digest(); h[64:72]=b'BRLCTL43'; h[72:80]=bytes.fromhex(o['source_sha256'])[:8]
...
def compile_src(src,out,factory=False,manifest=None,brir_out=None,sign_private=None,brctl=None):
    ...
    out = Path(out)
    if sign_private and not brctl:
        die('--brctl required with --sign-private')
    out.parent.mkdir(parents=True, exist_ok=True)
    signed = False
    # Complete signing before publishing any image or associated metadata.
    with tempfile.TemporaryDirectory(prefix='.lctl-sign-', dir=out.parent) as temp:
        staged = Path(temp) / 'image.brimg'
        if sign_private:
            unsigned = Path(temp) / 'unsigned.brimg'
            unsigned.write_bytes(img)
            subprocess.run([brctl, 'sign-image', str(unsigned), str(staged), sign_private],
                           check=True, timeout=60)
            ib = staged.read_bytes()
            expected = bytearray(img)
            expected[7] |= 2
            if len(ib) != len(img) + 64 or ib[:-64] != bytes(expected):
                die('signer output is not the requested signed image')
            signed = True
        else:
            ib = img
            staged.write_bytes(ib)
        os.replace(staged, out)
    if brir_out:
        Path(brir_out).write_bytes(bb+b'\n')
    man={'record':'BR.Provenance','v':V,'source':str(src),'source_sha256':o['source_sha256'],'brir_sha256':sha(bb),'brim_sha256':sha(ib),'payload_sha256':ib[32:64].hex(),'compiler_version':'br-lctlc/'+V,'verifier_version':'br-lctlv/'+V,'language_version':LANG,'isa_version':'BR/1.1','image_version':o['image_version'],'requested_caps':o['requested_caps'],'instruction_count':len(o['instructions']),'signed':signed,'header_source_hash_prefix':ib[72:80].hex()}
